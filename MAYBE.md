...
- Add per-job SMB/session retry and backoff controls for weak links (tailscale/hotspot).
...
- Keep the current behavior as default (lean, relies on the native engine); add knobs only if field failures appear.
