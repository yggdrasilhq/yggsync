...
curl -H "Authorization: token $GITEA_TOKEN" \
  https://git.example.test/api/v1/repos/yggdrasilhq/yggsync/releases | jq
```
...
RELEASE_ID=$(curl -s -H "Authorization: token $GITEA_TOKEN" \
  https://git.example.test/api/v1/repos/yggdrasilhq/yggsync/releases | \
  jq 'map(select(.tag_name=="v0.1.3"))[0].id')
curl -s -X POST \
  -H "Authorization: token $GITEA_TOKEN" \
  -H "Content-Type: multipart/form-data" \
  -F "attachment=@/tmp/yggsync-linux-amd64" \
  "https://git.example.test/api/v1/repos/yggdrasilhq/yggsync/releases/${RELEASE_ID}/assets?name=yggsync-linux-amd64"
```
